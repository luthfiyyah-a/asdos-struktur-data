...
void jumlahin_segaris(AVLNode *root, int target_posisi, int* jumlah) {
    if (root) {
...
int main(){
    ...
    int i, j;
    char command[20];
    int posisi_terkanan, posisi_terkiri; //untuk nandain posisi node yang paling kanan dan paling kiri di avl
    int jumlah;
    AVL avl;
    avl_init(&avl);
    
    scanf("%s", command);
    while(strcmp(command, "HITUNG") != 0){
    ...
    int total = 0;
	for(j=posisi_terkiri; j<=posisi_terkanan; j++){
	    jumlah = 0;
